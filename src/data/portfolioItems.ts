...
// Define fallback portfolio items
...
export async function getPortfolioItems(): Promise<PortfolioItem[]> {
  try {
    if (realPortfolioItems.length > 0) {
      return realPortfolioItems;
    }
    throw new Error('No real portfolio items available.');
  } catch (error) {
    console.warn('Error loading portfolio items. Using fallback items:', error);
...
export const portfolioItems: PortfolioItem[] =
  realPortfolioItems.length > 0 ? realPortfolioItems : fallbackPortfolioItems;
